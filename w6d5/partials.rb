# Partials
# Views can get really long and complicated. Just like you break up long methods into smaller ones, you can also break up views into smaller bits, called partials. Like small methods, partials are often reusable in different views, so they also help you keep your code DRY.
...
# Just like with methods, you can also pass local variables into partials, making them even more powerful and flexible. For example, you can use this technique to reduce duplication between new and edit pages, while still keeping a bit of distinct content:
...
# However convenient it may seem to just use instance variables when creating partials, remember that you want to make partials flexible and non-reliant on sources of data defined outside of their scope. This also simplifies your debugging process. Failing to provide a required instance variable can produce confusing errors; failing to define a local variable raises the unambiguous "no local variable defined" error.